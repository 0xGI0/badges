...
// ---------------------------------------------------------------------------
// Logo lookup tables — built lazily on first use and guarded.
//
// simple-icons ships a ~5 MB module and Octicons a large object; loading and
// indexing them is deferred to the first logo lookup so a logo-less badge
// never pays for it, and — crucially — any failure to load or index a logo
// library is contained here instead of throwing at module import time (which
// on serverless platforms surfaces as an uncatchable FUNCTION_INVOCATION_FAILED
// rather than a recoverable error the handler can turn into an error badge).
// ---------------------------------------------------------------------------

let siMap = null; // normalised slug/title -> simple-icons icon
let octMap = null; // normalised name -> octicon

function ensureLogos() {
  if (siMap && octMap) return;
  siMap = Object.create(null);
  octMap = Object.create(null);
  ...
  try {
    ...
    /* simple-icons unavailable: badges still render, just without those logos */
  }
  ...
  try {
    ...
    /* octicons unavailable: badges still render, just without those logos */
  }
}
