...
import octiconsPkg from '@primer/octicons';

// @primer/octicons is a CommonJS package; under ESM its export lands on the
// default (or, depending on interop, the namespace itself).
const octicons = octiconsPkg && octiconsPkg.default ? octiconsPkg.default : octiconsPkg;
...
// ---------------------------------------------------------------------------
// Logo lookup tables — built once, guarded.
//
// Indexing simple-icons (~3 300 icons) and Octicons is wrapped in try/catch so
// that a malformed icon entry degrades to a logo-less badge instead of taking
// the whole module (and thus the serverless function) down at import time.
// ---------------------------------------------------------------------------
...
try {
  for (const name of OCTICON_LOGOS) {
    if (octicons && octicons[name]) octMap[norm(name)] = octicons[name];
  }
} catch (_) {
...
export { buildBadge, logoDataUri, parsePath, cssColor };
